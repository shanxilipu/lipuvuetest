...
import { search, fetchResult } from '@/services/auditManagement/hostLogInquire';

export default {
  namespace: 'hostLogInquire',
  ...
  effects: {
    ...
    *search({ payload }, { call, put, select }) {
      const preArg = yield select(({ hostLogInquire: state }) => {
      ...
      const query = yield select(({ hostLogInquire: state }) => {
        const { form, pageIndex, pageSize } = state;
        return {
          ...form,
          pageIndex,
